...
function calculateLarger(elements, output) {
    console.log(elements)
    let increased = [];
    let decreased = [];
    let nothing = [];
    elements.map((e, index) => {
        if (index === 0 || (elements[index - 1] === elements[index])) {
            nothing.push(`${e} (nothing)`)
        } else if (elements[index - 1] < elements[index]) {
            increased.push(`${e} (Increased)`);
        } else {
            decreased.push(`${e} (Decresed)`);
        }
    });
    ...
    <p>Y ${nothing.length} permacecieron iguales o sin clasificar</p>
    `
}
...
function sliderWindow() {
    ...
    measurements.map((e, index) => {
        if (index < (measurements.length - 2)) {
            sliderWinResult[index] = measurements[index] + measurements[index + 1] + measurements[index + 2]
        }
    })
    calculateLarger(sliderWinResult, "step2")
}